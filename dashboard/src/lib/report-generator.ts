...
export function parseJSONLLogs(jsonlContent: string): ReportData {
  ...
  for (const event of events) {
    ...
    else if (eventType === 'detect-pagination' && action === 'llm-completion' && current_pagination) {
      if (event.status !== 'pending') {
        current_pagination.llm_calls.push(event);
      }
    }
    else if (eventType === 'code-generation' && action === 'llm-completion' && event.status !== 'pending') {
      if (!current_code_gen) {
        current_code_gen = { status: 'pending' };
      }
  ...
  if (analysis_begin && analysis_end && analysis_begin.timestamp && analysis_end.timestamp) {
    try {
      const startTime = parseISO(analysis_begin.timestamp.replace('Z', '+00:00'));
  ...
  if (analysis_end?.status === 'success' && typeof analysis_end.relevant_api_call === 'string') {
